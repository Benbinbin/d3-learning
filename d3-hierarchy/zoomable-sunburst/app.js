 ...
 */
  ...
  // 设置径向树布局参数
  radius = width > height ? height / (root.height + 2) : width / (root.height + 2); // 该参数并不是径向图的实际半径大小，而是环形的宽度（用以与相对坐标相乘，得到绝对坐标）
  ...
  const path = container.append("g")
    ...
    .attr("d", d => arc(d)) // 使用 d.current 未修改的节点数据生成环形路径
  ...
  function clicked(event, p) {
    parent.datum(p.parent || tree); // 更新旭日图中心圆绑定的数据，点击节点的父节点（如果点击的节点已经是根节点就绑定根节点对应的数据，由于根节点 parent 为 null）

    // 遍历树的所有节点，并基于与点击节点的相对关系（层级和偏移角度）重新计算 x0、x1、y0、y1，并将这些值存在 d.target 属性上，作为各个节点对应的环形（在该次点击后进行变换）最终状态（而 d.current 作为起始状态）
    // 然后结合函数 arcVisible() 和 labelVisible() 控制环形的透明度，实现环形的「隐藏/显示」
