...
function scatterPlot(
  ...
  pointsContainer
    ...
    .on("mouseover", function (event, d) {
      ...
      // 调整元素在纵坐标上的位置，并显示出来
      yForeign.style("opacity", 1).attr("y", yScale(Y[d]) - 15);
    })
...
function debounce(delay = 500) {
  ...
  timer = setTimeout(function () {
    ...
    if (w !== width || h !== height) {
       ...
       */
      // 先取消在数据点上设置的鼠标移动监听器
      pointsContainer.selectAll("circle").on("mouseover", null);

      // 使用新的比例尺计算高亮显示的值的位置
      xForeign.attr("transform", `translate(0, ${h - marginBottom / 2})`);

      pointsContainer.selectAll("circle").on("mouseover", function (event, d) {
        this.classList.add("activeDot");

        xDom.text(X[d]);
        yDom.text(Y[d]);

        xForeign.attr("x", newXScale(X[d]) - 25).style("opacity", 1);

        yForeign.style("opacity", 1).attr("y", newYScale(Y[d]) - 15);
      });
    }
